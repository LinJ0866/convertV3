...
def save_split_result(outputDir, dataset):
    ...
    for i in range(len(dataset)):
        with open(os.path.join(saved_path, text[i]+'.txt'), encoding='utf-8', mode='w+') as f:
            f.writelines('\n'.join(dataset[i]))

def dataset_split(paths, train, val, test, random_state=None, outputDir='', pre=''):
    """
    划分数据集
    ...
    Args:
        ...
        pre(str): 若pre为空，paths为文件夹名，否则paths为文件名
    Output：
        train=1: train_list
    ...
    if pre == '':
        ...
        for path in paths:
            pathList.append(os.path.join(pre, path))
